...
function countMoviesByYear(year) {
  let count = 0;
  for (const movie of ghibliFilms) {
    if (movie.year === year) {
      count++;
    }
  }
...
yearSelect.addEventListener("change", function () {
  ...
  if (selectedYear === 0) {
    ...
    const totalMovieCount = ghibliFilms.length;
...
function displayMovies() {
  ...
  const filteredMovies = ghibliFilms.filter((movie) => {
...
function createMovieCard(movie) {
  ...
  detailButton.href = `./detailghibli.html?title=${encodeURIComponent(
    movie.title
  )}`;
